...
#'@export
L2diff_gammaSeq = function(mLSE, spectrf = NULL, spectr.alt = NULL, method, w = NULL, dw = NULL){
  
  # Input: spectrf or spectr.alt for gamma, mLSE = Pi_d(r) for d in delta
  # spectr.alt should be a list containing type %in% c("L2seq","momentseq") and seq (either vector or SRfit1 object)
  # For method == "numInt", x = numerical integration 
  # Output: ||gamma(spectrf) - Pi_d(r) ||^2 
  
  nD = length(mLSE)
  v = numeric(nD) # initialize
  
  if(method=="exact"){
    if(is.null(spectr.alt)){stop("spectr.alt needs to be provided")}
    
    if(spectr.alt$type=="momentseq"){
      
      for(i in 1:length(v)){
        v[i] = L2diff_twoMoments(support1 = mLSE[[i]]$support,weights1 = mLSE[[i]]$weights, 
                                 support2 = spectr.alt$seq$support, weights2 = spectr.alt$seq$weights)
      }
      
    }else if(spectr.alt$type=="L2seq"){
      
      for(i in 1:length(v)){
        v[i] = L2diff_L2Moment(r = spectr.alt$seq, support = mLSE[[i]]$support, weights = mLSE[[i]]$weights)
      }
    ...
    if(is.null(spectrf)){stop("spectrf needs to be provided")}
    
    spectr = spectrf(w)
    for(i in 1:length(v)){
      FmLSE = momentSpectrDens(momentSeq = mLSE[[i]],FFreq = w)
      v[i] = sum((spectr - FmLSE)^2)*dw
    }
  }
  return(v)
}
